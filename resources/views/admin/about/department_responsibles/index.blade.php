<x-layouts.app>
    ...
    <div class="flex justify-between items-center my-5">
        <flux:heading size="lg">All Department Responsibles </flux:heading>
        <a href="{{ route('department_responsibles.create') }}"
        ...
        <table class="w-full text-left text-sm text-slate-700 dark:text-slate-300">
            <thead
                ...
                <tr>
                    ...
                    <th scope="col" class="p-4">Department email</th>
                    <th scope="col" class="p-4">Contact No</th>
                    <th scope="col" class="p-4">Action</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-200 dark:divide-zinc-800">
                @foreach ($department_responsibles as $item)
                    <tr class="even:bg-slate-50 dark:even:bg-zinc-700">
                        ...
                        <td class="p-4">
                            <div class="flex gap-2">
                                <flux:button href="{{ route('department_responsibles.edit', $item->id) }}" variant="primary">Edit</flux:button>
                                <form action="{{ route('department_responsibles.destroy', $item->id) }}" method="post"
                                    class="inline-block">
                                    @csrf
                                    @method('DELETE')

<x-layouts.app>
    ...
    <div class="flex justify-between items-center my-5">
        <flux:heading size="lg">All Board of Director Messages</flux:heading>
    ...
    <div x-data="{ selectedId:'' }" class="overflow-hidden w-full overflow-x-auto rounded-xl border border-slate-300 dark:border-slate-700">
        ...
        <table class="w-full text-left text-sm text-slate-700 dark:text-slate-300">
            <thead
                ...
                <tr>
                    <th scope="col" class="p-4">No Of Director</th>
                    <th scope="col" class="p-4">Photo</th>
                    <th scope="col" class="p-4">Director name</th>
                    <th scope="col" class="p-4">Director email</th>
                    <th scope="col" class="p-4">Messages</th>
                    <th scope="col" class="p-4">Action</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-200 dark:divide-zinc-800">
                @foreach ($board_of_director as $item)
                    <tr class="even:bg-slate-50 dark:even:bg-zinc-700">
                        ...
                        <td class="p-4">{!! $item->no_of_director !!}</td>
                        ...
                        <td class="p-4">
                            <div class="flex gap-2">
                                <flux:button href="{{ route('board_of_director.edit', $item->id) }}" variant="primary">Edit</flux:button>
                                <form action="{{ route('board_of_director.destroy', $item->id) }}" method="post"
                                    class="inline-block">
                                    @csrf
                                    @method('DELETE')

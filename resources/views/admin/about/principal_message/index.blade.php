<x-layouts.app>
    ...
    <div class="flex justify-between items-center my-5">
        <flux:heading size="lg">All Principal Messages</flux:heading>
        <a href="{{ route('principal_message.create') }}"
    ...
    <div x-data="{ selectedId:'' }" class="overflow-hidden w-full overflow-x-auto rounded-xl border border-slate-300 dark:border-slate-700">
        ...
        <table class="w-full text-left text-sm text-slate-700 dark:text-slate-300">
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-zinc-800">
                @foreach ($principal_message as $item)
                    <tr class="even:bg-slate-50 dark:even:bg-zinc-700">
                        ...
                        <td class="p-4">
                            <div class="flex gap-2">
                                <flux:button href="{{ route('principal_message.edit', $item->id) }}" variant="primary">Edit</flux:button>
                                <form action="{{ route('principal_message.destroy', $item->id) }}" method="post"class="inline-block">
                                    @csrf
                                    @method('DELETE')

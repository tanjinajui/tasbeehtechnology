<x-layouts.app>
    <div class="flex flex-col gap-6">
        <x-auth-header title="Create Principal Message" description="" />

        <!-- Session Status -->
        <x-auth-session-status class="text-center" :status="session('status')" />

        <form method="POST" action="{{ route('principal_message.store') }}" class="flex flex-col gap-6"
            enctype="multipart/form-data">
            @csrf
            @method('POST')

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <flux:input id="photo" :label="__('Principal Photo')" type="file" name="photo"
                    accept="image/*" />
            ...
            <div>
                <flux:textarea class="message" :label="__('Principal Message')" type="text" name="message" placeholder="Principal Message" value="{{ old('message') }}" />

<x-layouts.app>
    <div class="flex flex-col gap-6">
        <x-auth-header title="Update Principal Messages" description="" />

        <!-- Session Status -->
        <x-auth-session-status class="text-center" :status="session('status')" />

        <form method="POST" action="{{ route('principal_message.update', $principal_message->id) }}" class="flex flex-col gap-6"
            enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="">
                <img id="photo" class="img-thumbnail img-fluid"  src="{{asset('/storage/'. $principal_message->photo)}}" alt="" width="100px">
            ...
            <div>
                {{-- <flux:textarea class="message" :label="__('Principal Message')" name="message" 
                    ...
                    <textarea  class="message"  name="message" placeholder="Principal Message">{{ old('message', $principal_message->message) }}</textarea>
            </div>
